...
public class DraggablePanel extends FrameLayout{
	...
	public void setLockDragMode(boolean lock){
		if(initialised){ draggableView.setLockDragMode(lock); } else{
	...
	public boolean isLockDragMode(){
		if(initialised){ return draggableView.isLockDragMode(); } else{
			throw new IllegalStateException("You have to initialise draggable panel first");
		}
	...
	private void initializeAttrs(AttributeSet attrs){
		TypedArray attributes =
			getContext().obtainStyledAttributes(attrs, R.styleable.draggable_panel);
		...
		this.enableTouchListener = attributes.getBoolean(
			R.styleable.draggable_panel_enable_touch_listener_panel, DEFAULT_ENABLE_TOUCH_LISTENER
		);
		attributes.recycle();
	}
